...
static uint8 CONFIG_INITtx=        	(EN_CRC | CRC0 | PWR_UP ); //!< enabled RX_DR, TX_DS and MAX_RT interrupts; enabled CRC (2 bytes); device powered up and in RX mode
...
void RF24send5bytes( uint8 byte1, uint8 byte2, uint8 byte3, uint8 byte4, uint8 byte5 )
{
	 SPI_SendByte(FLUSH_TX); /* flush old data */
	 pin_CELOW();  /* back to normal */
	 Write5RegRf(W_TX_PAYLOAD,byte1,byte2,byte3,byte4,byte5);
	 pin_CEHIGH();/* start transmission */
	 Pause(10000);  //40ms /* keep signal high for 40 micro-seconds */
	 pin_CELOW();  /* back to normal */
}

uint8 apagarbanderaydatoenviado(uint8 status1)
{
	uint8 datasent=0;//1
	if (status1&RX_DR) /* data received interrupt */
	{
	...
	if (status1&TX_DS) /* data sent interrupt */
	{
   	 pin_CELOW(); /* need to disable rx mode during reading RX data */
    ...
    if (status1&MAX_RT) /* retry timeout interrupt */
    {
   	 pin_CELOW(); /* need to disable rx mode during reading RX data */  
